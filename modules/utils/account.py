...
from starknet_py.net.account.account import _parse_calls, _merge_calls
...
class Account(BaseAccount): #TODO: combine get_balance_evm and get_balance_starknet to get_balance
    ...
    async def setup_account(self):
        ...
        self.stark_address = await self.get_starknet_address_from_private_async(self.private_key)
    ...
    async def get_invocation(self, calls):
        ...
        while i <= SETTINGS["RetriesLimit"]:
            ...
            try:
                ...
                wrapped_calldata = _parse_calls(await self.stark_native_account.cairo_version, calls)
    ...
    def get_starknet_address_from_private(private_key: str):
        ...
        while True:
            try:
                ...
                if len(addrs) > 1:
                    print(addrs)
                PUBLIC_KEYS_PAIRS[pub_key] = normalize_to_32_bytes(addrs[len(addrs)-1])
                return normalize_to_32_bytes(addrs[len(addrs)-1])
                    
            except Exception as e:
                console_log.error(f"can't get address:{e} trying again")
                sync_sleep(5)
    
    async def get_starknet_address_from_private_async(self, private_key: str):
        key_pair =  KeyPair.from_private_key(int(private_key, 16))
        pub_key = normalize_to_32_bytes(hex(key_pair.public_key))
        if pub_key in list(PUBLIC_KEYS_PAIRS.keys()):
            return normalize_to_32_bytes(PUBLIC_KEYS_PAIRS[pub_key])
        ...
        while True:
            try:
                async with ClientSession() as session: 
                    async with session.get(f"{base_link}{pub_key}") as resp:
                        addrs = (await resp.json())["address"]
                if len(addrs) == 0:
                    return Account.create_starknet_address(private_key)
                if len(addrs) > 1:
                    print(addrs)
